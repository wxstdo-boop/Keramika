...
import 'utils/android_settings.dart' show setSecureWindow;
...
Future<void> _autoExportTick({bool silent = false}) async {
  ...
  try {
    await _performAutoExport(showSnack: !silent);
    globalPrefs.setInt(
      _lastAutoExportKey,
...
Future<String> saveBackupBytes(List<int> bytes, String fileName) async {
  ...
  if (Platform.isAndroid) {
    try {
      final path = await _backupChannel.invokeMethod<String>(
        'saveToDownloads',
        {
          'fileName': fileName,
...
class KeramikaAppState extends State<KeramikaApp>
  ...
  void _goHome() {
    // Снимаем FLAG_SECURE (показываем содержимое в переключателе задач
    // можно ТОЛЬКО когда PIN разблокирован — иначе там видны личные записи).
  ...
  void _goLock() {
    ...
    // в обход блокировки) и закрываем содержимое в «недавних» — Android
    // покажет размытую заглушку вместо списка привычек/питания.
