...
/// Размеры окна оверлея (dp).
const double _bubbleSize = 72;
const double _chatWidth = 300;
const double _chatHeight = 520;
...
class _AdaOverlayAppState extends State<AdaOverlayApp> {
  ...
  Future<void> _init() async {
    ...
    setState(() {
      _lang = lang;
      _themeDark = theme == 'dark' || theme == 'darkGrok' || theme == 'system';
      _ready = true;
    });
  }
...
class _OverlayChatState extends State<_OverlayChat>
  ...
  void _startDrift() {
    _driftTimer?.cancel();
    _ensurePos();
    _vx = Random().nextBool() ? 0.8 : -0.8;
    _vy = Random().nextBool() ? 0.6 : -0.6;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final appearT = Curves.easeOutBack.transform(_appearCtrl.value);
  ...
  Widget _buildChatShell() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        // Тот же «пузырьковый» стиль, что у мини-окошека на web:
        // градиент корпуса, белая обводка, цветное свечение + тень.
      ...
      child: Stack(
        ...
        children: [
          ...
          // Глянцевый блик сверху — как у web-пузыря.
          Positioned.fill(
            child: IgnorePointer(
              child: DecoratedBox(
                decoration: BoxDecoration(
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Row(
          ...
          children: [
            if (!isUser) ...[
              _OverlayAvatar(size: 20, variant: avatar),
              const SizedBox(width: 5),
            ],
            Flexible(
              child: Container(
