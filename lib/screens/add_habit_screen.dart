...
class _AddHabitScreenState extends State<AddHabitScreen> {
  ...
  /// Секцию «Вспомнить всё» скрываем, если напоминание уже установлено
  /// на ДРУГОЙ привычке: одновременно может стоять только одно напоминание.
  bool get _reminderSectionHidden {
    if (_reminderTime != null) return false; // у этой привычки — показываем
    return HabitService()
        .habits
        .any((h) => h.id != widget.existing?.id && h.reminderTime != null);
  }
  ...
  void _save() {
    ...
    final habit = Habit(
      ...
      reminderText: _reminderText?.trim().isEmpty == true
          ? null
          : _reminderText?.trim(),
    );
        ...
        body: SmoothKeyboardBody(
          child: ListView(
            ...
            children: [
              ...
              // Напоминание может стоять ТОЛЬКО на ОДНОЙ привычке: если оно
              // уже установлено на другой — у этой привычки секция
              // «Вспомнить всё» не показывается вообще.
              if (_reminderSectionHidden)
                const SizedBox.shrink()
              else
                _buildReminderFullCard(context, theme),
            ],
          ),
        ),
      ),
  ...
  /// Полная секция «Вспомнить всё»: время + текст напоминания.
  /// Видна ТОЛЬКО у привычки, где напоминание уже установлено.
  Widget _buildReminderFullCard(BuildContext context, ThemeData theme) {
    return SmoothHover(
                  hoverScale: 1.01,
                  child: Card(
                    shape: RoundedRectangleBorder(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              TextField(
                                                ...
                                                maxLines: 1,
                                                buildCounter:
                                                    smoothCharCounterBuilder,
                                                // Как во ВСЕХ остальных полях
                                                // приложения: без лупы при
                                                // выделении и без «троеточия»/
                                                // «Спросить копилот» в меню
                                                // выделения — только копировать/
                                                // вставить/вырезать.
                                                magnifierConfiguration:
                                                    TextMagnifierConfiguration.disabled,
                                                contextMenuBuilder:
                                                    minimalContextMenuBuilder,
                                                decoration: InputDecoration(
              ...
              children: [
                AnimatedSwitcher(
                  ...
                  child: selected
                      ...
                      : const SizedBox(key: ValueKey('day_off'), width: 14),
                ),
