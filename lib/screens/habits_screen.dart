...
class _HabitsScreenState extends State<HabitsScreen>
  ...
  // Id привычки, которая только что закрепилась — плавная «вспышка».
  String? _pinFlashId;
  Timer? _dayTimer;
  ...
  void _handleHabitTap(Habit habit) {
    ...
    if (_tapCount >= 4) {
      ...
      // Build a fresh Habit so lastDoneDate is actually null —
      // copyWith uses `?? this.lastDoneDate` which silently ignores null.
      final reset = habit.copyWith(streak: 0, doneToday: false);
      reset.lastDoneDate = null;
      _service.update(reset);
      Haptics.select();
      if (mounted) {
        showBeautifulSnackBar(
          context,
  ...
  void _togglePin(Habit habit) async {
    ...
    if (!mounted) return;
    // Плавное перемещение: список кросс-фейдится в новую раскладку
    // (закреплённая карточка оказывается сверху, не прыгая), плюс
    // «вспышка» на самой карточке.
    setState(() {
      _pinRev++;
      _pinFlashId = habit.id;
    });
    Future.delayed(const Duration(milliseconds: 650), () {
      if (mounted && _pinFlashId == habit.id) {
        setState(() => _pinFlashId = null);
      }
    });
    showBeautifulSnackBar(
      context,
            ...
            body: FabScrollListener(
              ...
              child: Column(
                children: [
                  ...
                  // «Сделано с» всегда внизу экрана (и при пустом списке).
  ...
  Widget _buildHabitCard(
    ...
    // Плавная «вспышка» закрепления: карточка мягко увеличивается
    // и проясняется на новом месте (вместо резкого прыжка позиции).
    final flashing = _pinFlashId == habit.id;
    ...
    // Плавное «вплывание» закреплённой карточки на новом месте.
